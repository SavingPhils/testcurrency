...
#if defined(NDEBUG)
#error "Testcurrency cannot be compiled without assertions."
#endif
...
{
    int64_t ret = blockValue*65; // start at 65%
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Testcurrency
...
{
    RenameThread("testcurrency-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZTESTMinted()
{
...
void RecalculateZTESTSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTEST supply
...
bool RecalculateTESTSupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TEST to zTEST
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Testcurrency: We use certain sporks during IBD, so check to see if they are
