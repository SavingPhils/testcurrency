...
#define URI_SCHEME "testcurrency"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Testcurrency address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no Testcurrency: URI
...
{
    // Convert testcurrency:// to testcurrency:
    //
    //    Cannot handle this later, because testcurrency:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open testcurrency.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Testcurrency.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Testcurrency.lnk
...
{
    return GetAutostartDir() / "testcurrency.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a testcurrency.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Testcurrency\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the testcurrency app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add testcurrency app to startup item list
