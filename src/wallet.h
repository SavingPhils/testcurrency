...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TEST at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTEST send
enum ZerocoinSpendStatus {
    ZTEST_SPEND_OKAY = 0,                            // No error
    ZTEST_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTEST_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTEST_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTEST_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTEST_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTEST_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTEST_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTEST_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTEST_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTEST_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTEST_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTEST_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTEST_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTEST_SPENT_USED_ZTEST = 14,                      // Coin has already been spend
    ZTEST_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
